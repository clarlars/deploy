...
if ( window.odkDataIf === null || window.odkDataIf === undefined ) {
  window.odkDataIf = {
	...
	_execute: function( json ) {
		...
		xhr.open('POST', '/webservice/OdkDataHostIf', true);
		...
		xhr.onload = function() {
			if ( this.status === 200 ) {
				...
				that._queueResponse( JSON.stringify({ error: "unable to access server or returned something other than 200" }) );
			}
		};
    ...
    getViewData : function (_callbackId, limit, offset) {
		...
		if ( window.parent !== window ) {
			viewDataParams = window.parent.getViewDataParameters(window.frameElement.odkViewId);
		}
    ...
    query: function(tableId, whereClause, sqlBindParamsJSON, groupBy, having,
		...
		var json = { request: "query",
					 ...
					 sqlSelectionArgs: sqlBindParamsJSON,
					 sqlGroupByArgs: groupBy,
					 sqlHavingClause: having,
					 sqlOrderByElementKey: orderByElementKey,
					 sqlOrderByDirection: orderByDirection,
					 limit: limit,
					 offset: offset,
    ...
    changeAccessFilterOfRow: function(tableId, defaultAccess, rowOwner, groupReadOnly, groupModify,
        groupPrivileged, rowId, metaDataRev, _callbackId) {
        var that = this;
