...
/**
 * Attempt to interpret an intent description as a survey intent, and return the
...
public class OdkSurveyIntentParser {
   ...
   *
   * /appName/_ID/ use the _ID column to retrieve the referenced form this is a
   * numeric row id.
   *
   * or
   *
   * /appName/tableId/ this will return all forms for a given tableId.
   *
   * or
   *
   * /appName/tableId// this requires a call to updatePatchedFilter(...) to
   * retrieve and use the default formId for this tableId. If there is no
   * configured default formId, use the tableId as the formId.
   *
   * or
   *
   ...
   * @return
   */
  private PatchedFilter extractUriFeatures(Uri uri, List<String> segments, String where,
      String[] whereArgs) {
    ...
    // Modify the where clause to account for the presence of any additional
    // segments
    if (segments.size() == 1) {
      ...
      if (TextUtils.isEmpty(where)) {
        ...
        pf.whereId = (pf.isNumericFormId ? BaseColumns._ID : FormsColumns.TABLE_ID) + "=? AND ("
            + where + ")";
  ...
  private void updatePatchedFilter(OdkConnectionInterface db, PatchedFilter pf) {
    if (pf.requiresBlankFormIdPatch) {
      TableMetaDataEntries values = ODKDatabaseImplUtils.get().getTableMetadata(db, pf.tableId,
  ...
  private final String retrieveFormPath(String appName, Uri formUri) {
    ...
    try {
      ...
      c = db.query(DatabaseConstants.FORMS_TABLE_NAME, null, pf.whereId, pf.whereIdArgs, null, null,
          null, null);
      ...
      if (c != null && c.getCount() == 1) {
        ...
        // File formDefJsonFile = new File(formDirectory,
      ...
      if (db != null) {
        try {
          ...
          if (!success) {
            ...
            // if it was successful, then the InvalidateMonitor will close the
            // connection
      ...
      if (uri.getScheme().equalsIgnoreCase(uriFormsProvider.getScheme())
        ...
        if (segments != null && segments.size() >= 2) {
          ...
          if (!appName.equals(thisAppName)) {
            throw new IllegalArgumentException(
                "appName does not match expected value: " + thisAppName);
          }
          ...
          if (formPath != null) {
            ...
            try {
              ...
              retVal
                  .put("url",
                      appNameUrlPrefix + appName + "/"
                          + ODKFileUtils.asUriFragment(appName,
                              new File(ODKFileUtils.getSystemFolder(appName), "index.html"))
                          + hashUrl);
  ...
  public Map<String, Object> getFrameworkUrl() {
    ...
    /*
     * // we want framework... File frameworkFormDef = new File(
     * ODKFileUtils.getFormFolder(appName, FormsColumns.COMMON_BASE_FORM_ID,
     * FormsColumns.COMMON_BASE_FORM_ID), "formDef.json");
     * 
     * String hashUrl = "#formPath=" +
     * FormsProviderUtils.encodeFragmentUnquotedStringValue(ODKFileUtils.
     * getRelativeFormPath(appName, frameworkFormDef)) + ((instanceId == null) ?
     * "" : "&instanceId=" +
     ...
     * ("&refId=" + FormsProviderUtils.encodeFragmentUnquotedStringValue(refId))
     * + ((auxillaryHash == null) ? "" : "&" + auxillaryHash); return hashUrl;
     * 
     */
    ...
    retVal.put("url", appNameUrlPrefix + appName + "/" + ODKFileUtils.asUriFragment(appName,
        new File(ODKFileUtils.getSystemFolder(appName), "index.html")) + hashUrl);
