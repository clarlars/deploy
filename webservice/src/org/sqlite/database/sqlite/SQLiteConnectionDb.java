 ...
 * @hide
 */
public final class SQLiteConnectionDb extends SQLiteConnectionBase implements CancellationSignal.OnCancelListener {
   private static final String TAG = "SQLiteConnection";
   private static final boolean DEBUG = false;
   
   private static final String driverPrefix = JDBC.PREFIX;
   ...
   private static final class PreparedStatement {
      ...
      public java.sql.PreparedStatement mStatementPtr;
    ...
    * <em>Should be accessed within the mConnectionPtrMutex lock</em>
    */
   private Connection mConnectionPtr = null;
   ...
   public SQLiteConnectionDb(SQLiteDatabaseConfiguration configuration,
      ...
      synchronized (mConnectionPtrMutex) {
         ...
         try {
            ...
            try {
               ...
               try {
                 java.sql.ResultSet resultSet = statement.mStatementPtr.executeQuery();
                 /*
                  * construct sqliteContent -- an array of object arrays. First row
                 ...
                 for ( int i = 0 ; i < nCols ; ++i ) {
                   ...
                   case java.sql.Types.TINYINT:
                     colTypes[i] = SQLiteMemoryCursor.LONG_TYPE;
                     break;
                   case java.sql.Types.FLOAT:
                   case java.sql.Types.DECIMAL:
                   case java.sql.Types.DOUBLE:
                   case java.sql.Types.NUMERIC:
                     colTypes[i] = SQLiteMemoryCursor.DOUBLE_TYPE;
                     break;
                   ...
                   default:
                     colTypes[i] = SQLiteMemoryCursor.STRING_TYPE;
                     break;
                   case java.sql.Types.NULL:
                     colTypes[i] = SQLiteMemoryCursor.NULL_TYPE;
                     break;
                   case java.sql.Types.LONGVARBINARY:
                   case java.sql.Types.VARBINARY:
                     colTypes[i] = SQLiteMemoryCursor.BYTEARRAY_TYPE;
                     break;
                   }
                 }
                 
                 resultArray.add(colNames);
                 resultArray.add(colTypes);
                 
                 while (hasResults) {
                   Object[] values = new Object[nCols];
                   for ( int i = 0 ; i < nCols ; ++i ) {
                     switch ( (char) colTypes[i]) {
                     case SQLiteMemoryCursor.LONG_TYPE:
                       values[i] = resultSet.getLong(i+1);
                       break;
                     case SQLiteMemoryCursor.DOUBLE_TYPE:
                       values[i] = resultSet.getDouble(i+1);
                       break;
                     default:
                     case SQLiteMemoryCursor.STRING_TYPE:
                       values[i] = resultSet.getString(i+1);
                       break;
                     case SQLiteMemoryCursor.BYTEARRAY_TYPE:
                       values[i] = resultSet.getBytes(i+1);
                       break;
                     }
           ...
           getLogger().e(TAG, "Exception during executeForCursor(): " + e.toString());
           getLogger().printStackTrace(e);
           mRecentOperations.failOperation(cookie, e);
           // TODO: use some other exception class?
           throw new IllegalArgumentException("Exception during executeForCursor()", e); 
         } catch (Throwable t) {
    ...
    */
   public boolean insert(String table, String nullColumnHack, Map<String,Object> values) {
      try {
   ...
   public void insertWithOnConflict(String table, String nullColumnHack,
      ...
      executeForLastInsertedRowIdImpl(sql.toString(), bindArgs, null);
   }
   ...
   private void openImpl() {
      ...
      synchronized (mConnectionPtrMutex) {
         ...
         // force load
         Class<Driver> theDriverClass;
         Driver theDriver = null;
         try {
           theDriverClass = (Class<Driver>) Class.forName("org.sqlite.JDBC");
           theDriver = theDriverClass.newInstance();
         } catch ( Exception e ) {
           // ignore this but log a brief info message
           getLogger().i(TAG,"Failed to load com.mysql.jdbc.Driver (did you download and install/copy MySQL Connector/J ?) Exception: " + e.toString());
         }

         try {
          mConnectionPtr = DriverManager.getConnection("jdbc:sqlite:" + mConfiguration.path.replaceAll("\\\\", "/"));
        } catch (Throwable t) {
          Enumeration<Driver> end = DriverManager.getDrivers();
          while ( end.hasMoreElements() ) {
            Driver driver = end.nextElement();
            getLogger().e(TAG, driver.toString());
          }
          try {
            theDriver.connect("jdbc:sqlite:" + mConfiguration.path.replaceAll("\\\\", "/"), null);
          } catch (java.sql.SQLException e) {
            e.printStackTrace();
          }
          getLogger().e(TAG, "Unable to open connection: " + t.toString());
          getLogger().printStackTrace(t);
          // TODO: use some other exception class?
          throw new IllegalArgumentException("unable to open database", t); 
        }
   ...
   private void setJournalMode(String newValue) {
      String value = null;
      for ( int i = 0 ; i < 3 ; ++i ) {
        value = executeForAltStringImpl("PRAGMA journal_mode", null, null);
        if ( value != null ) {
          break;
        }
      }
   ...
   public void onCancel() {
      synchronized (mConnectionPtrMutex) {
        ...
        try {
          mConnectionPtr.abort(mExecutor);
        } catch (java.sql.SQLException e) {
          getLogger().e(TAG, "Exception during onCancel() on open connection: " + e.toString());
          getLogger().printStackTrace(e);
          // TODO: use some other exception class?
          throw new IllegalArgumentException("Exception during onCancel() on open connection", e); 
        }
      }
   }
      ...
      synchronized (mConnectionPtrMutex) {
         ...
         try {
            ...
            try {
               ...
               try {
                 // nativeExecute
                 statement.mStatementPtr.execute();
               } finally {
                  detachCancellationSignal(cancellationSignal);
               }
            } finally {
           ...
           getLogger().e(TAG, "Exception during executeImpl(): " + e.toString());
           getLogger().printStackTrace(e);
           mRecentOperations.failOperation(cookie, e);
           // TODO: use some other exception class?
           throw new IllegalArgumentException("Exception during executeImpl()", e); 
         } catch (Throwable t) {
            mRecentOperations.failOperation(cookie, t);
            throw t;
         } finally {
   ...
   private long executeForLongImpl(String sql, Object[] bindArgs,
      ...
      synchronized (mConnectionPtrMutex) {
         ...
         try {
           ...
           getLogger().e(TAG, "Exception during executeForLongImpl(): " + e.toString());
           getLogger().printStackTrace(e);
           mRecentOperations.failOperation(cookie, e);
           // TODO: use some other exception class?
           throw new IllegalArgumentException("Exception during executeForLongImpl()", e); 
         } catch (Throwable t) {
           mRecentOperations.failOperation(cookie, t);
            throw t;
         } finally {
    ...
    * @throws OperationCanceledException if the operation was canceled.
    */
   private String executeForAltStringImpl(String sql, Object[] bindArgs,
       CancellationSignal cancellationSignal) {
      if (sql == null) {
         throw new IllegalArgumentException("sql must not be null.");
      }
         ...
         try {
            ...
            try {
               ...
               try {
                 ...
                 java.sql.ResultSet answer = statement.mStatementPtr.executeQuery();
                 if ( answer != null && answer.next() ) {
                   response = answer.getString(1);
                 }
                 if ( response == null ) {
                   String j = response;
                 }
           ...
           getLogger().e(TAG, "Exception during executeForStringImpl(): " + e.toString());
           getLogger().printStackTrace(e);
           mRecentOperations.failOperation(cookie, e);
           // TODO: use some other exception class?
           throw new IllegalArgumentException("Exception during executeForStringImpl()", e); 
         } catch (Throwable t) {
            mRecentOperations.failOperation(cookie, t);
            throw t;
         } finally {
         ...
         try {
            ...
            try {
               ...
               try {
                  ...
                  changedRows = statement.mStatementPtr.executeUpdate();
                  return changedRows;
               } finally {
                  detachCancellationSignal(cancellationSignal);
               }
           ...
           getLogger().e(TAG, "Exception during executeForChangedRowCountImpl(): " + e.toString());
           getLogger().printStackTrace(e);
           mRecentOperations.failOperation(cookie, e);
           // TODO: use some other exception class?
           throw new IllegalArgumentException("Exception during executeForChangedRowCountImpl()", e); 
         } catch (Throwable t) {
            mRecentOperations.failOperation(cookie, t);
            throw t;
         } finally {
    ...
    * @throws OperationCanceledException if the operation was canceled.
    */
   private void executeForLastInsertedRowIdImpl(String sql, Object[] bindArgs,
       CancellationSignal cancellationSignal) {
      if (sql == null) {
         throw new IllegalArgumentException("sql must not be null.");
      }
      ...
      if (cancellationSignal != null) {
         ...
         synchronized (mConnectionPtrMutex) {
            ...
            if (mCancellationSignalAttachCount == 1) {
               ...
               // TODO: any equivalent to nativeResetCancel ?
               // seems like we need the statementPtr to be able to do this here...
               try {
                 mConnectionPtr.clearWarnings();
               } catch (java.sql.SQLException ex) {
                 getLogger().e(TAG, "detachCancellationSignal() exception");
                 getLogger().printStackTrace(ex);
                 throw new IllegalArgumentException("detachCancellationSignal() exception", ex);
               }
   ...
   private void bindArguments(PreparedStatement statement, Object[] bindArgs) {
      ...
      try {
        final java.sql.PreparedStatement statementPtr = statement.mStatementPtr;
        for (int i = 0; i < count; i++) {
           final Object arg = bindArgs[i];
           switch (getTypeOfObject(arg)) {
           case Cursor.FIELD_TYPE_NULL:
             // TODO: confirm i+1 index and definition of data type?
             statementPtr.setNull(i+1, java.sql.Types.NULL);
             break;
           case Cursor.FIELD_TYPE_INTEGER:
             statementPtr.setLong(i+1, 
                  ((Number)arg).longValue());
              break;
           case Cursor.FIELD_TYPE_FLOAT:
             statementPtr.setDouble(i+1,
                  ((Number)arg).doubleValue());
              break;
           case Cursor.FIELD_TYPE_BLOB:
             statementPtr.setBytes(i+1, (byte[])arg);
              break;
        ...
        getLogger().e(TAG, "bindArguments() exception");
        getLogger().printStackTrace(ex);
        throw new IllegalArgumentException("bindArguments() exception", ex);
      }
   }
   ...
   public void dump(StringBuilder b, boolean verbose) {
      ...
      if (verbose) {
         synchronized (mConnectionPtrMutex) {
            b.append("connectionPtr: ").append(mConnectionPtr.toString()).append("\n");
         }
  ...
  private static class LinkedHashMapImpl extends LinkedHashMap<String, PreparedStatement> {
   ...
   private final class PreparedStatementCache {
         ...
         public void dump(StringBuilder b) {
            ...
            if (!cache.isEmpty()) {
               ...
               for (Map.Entry<String, PreparedStatement> entry : cache.entrySet()) {
                  ...
                  b.append("    ").append(i).append(": statementPtr=")
                      .append(statement.mStatementPtr.toString()).append(", numParameters=")
      ...
      PreparedStatement acquirePreparedStatement(String sql) {
         synchronized (impl) {
            ...
            try {
              ...
              final java.sql.PreparedStatement statementPtr = mConnectionPtr.prepareStatement(sql);
              try {
                 final java.sql.ParameterMetaData metadata = statementPtr.getParameterMetaData();
                 final int numParameters = (metadata != null) ? metadata.getParameterCount() : 0;
                 final int type = getSqlStatementType(sql);
                 ...
                 statement = obtainPreparedStatement(sql, statementPtr, numParameters, type);
                 ...
                 if ( ref == statement ) {
                   ...
                   // TODO: verify mapping to nativeFinalizeStatement()
                   statement.mStatementPtr.close();
                 }
                 throw ex;
              }
            } catch ( java.sql.SQLException e) {
              getLogger().e(TAG, "acquirePreparedStatement() exception");
              getLogger().printStackTrace(e);
              throw new IllegalArgumentException("acquirePreparedStatement() exception", e);
            }
            return statement;
         }
      }
      ...
      void releasePreparedStatement(PreparedStatement statement) {
         synchronized (impl) {
            ...
            if (existing == statement) {
               try {
                  if ( mConnectionPtr != null && statement.mStatementPtr != null ) {
                    // TODO: verify mapping to nativeResetStatementAndClearBindings()
                    statement.mStatementPtr.clearParameters();
                  }
       ...
       * @return
       */
      private PreparedStatement obtainPreparedStatement(String sql, java.sql.PreparedStatement statementPtr,
          int numParameters, int type) {
         synchronized (impl) {
