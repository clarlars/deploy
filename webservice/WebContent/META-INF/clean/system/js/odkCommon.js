...
if ( window.odkCommonIf === undefined || window.odkCommonIf === null ) {
    window.odkCommonIf = {
		...
		_getQueuedActions: function() {
			if ( window.parent === window ) {
				return this._notEmbeddedQueuedActions;
			} else {
				return window.parent.getQueuedActions();
			}
		},
        getPlatformInfo : function() {
            var that = this;
			if ( window.parent === window ) {
				...
				var platformInfo = {
					...
					activeUser: that.getActiveUser(),
					logLevel: that._logLevel,
				...
				return window.parent.getPlatformInfo(that._computeBaseUri(), that._logLevel);
			}
        },
        getFileAsUrl: function(relativePath) {
            var that = this;
        ...
        getRowFileAsUrl: function(tableId, rowId, relativePath) {
			...
			var iapp = baseUri.indexOf('/app/');
			var dataBaseUri = baseUri.substring(0,iapp) + '/scratch/' + odkCommon.getActiveUser() + baseUri.substring(iapp);
            ...
            if ( relativePath.length > prefix.length && relativePath.substring(0,prefix.length) === prefix ) {
                ...
                result = dataBaseUri + relativePath;
            } else {
                result = dataBaseUri + prefix + relativePath;
            }
        ...
        getActiveUser: function() {
            ...
            return window.parent.getActiveUser();
        },

        getProperty: function(propertyId) {
            this.log('D','odkCommon: DO: getProperty(' + propertyId + ')');
            return window.parent.getProperty(propertyId);
        },
        ...
        setSessionVariable: function(elementPath, value) {
            ...
            var i;
            var pterm = window.parent.getSessionVariables();
            var term = pterm;
        ...
        getSessionVariable: function(elementPath) {
            ...
            var parts = elementPath.split('.');
            var i;
            var term = window.parent.getSessionVariables();
            for ( i = 0 ; i < parts.length ; ++i ) {
                if ( parts[i] in term ) {
                    term = term[parts[i]];
                } else {
        ...
        doAction: function(dispatchStructAsJSONstring, action, jsonObj ) {
            ...
            if ( action === 'org.opendatakit.survey.activities.GeoPointActivity' ) {
				...
				// send request to server for interpretation
                value = JSON.parse(jsonObj);
				window.parent.odkDoAction(dispatchStruct, action, value, window.location);
                return "OK";
            }
        },
        closeWindow: function( resultCode, jsonResult ) {
			...
			if ( window.parent === window ) {
				...
				window.parent.odkCloseWindowAction(parseInt(resultCode), JSON.parse(keyValueBundle));
			}
		},
        /**
         ...
         */
        viewFirstQueuedAction: function() {
            if ( this._getQueuedActions().length !== 0 ) {
                var result = this._getQueuedActions()[0];
                return result;
            }
            return null;
        },
